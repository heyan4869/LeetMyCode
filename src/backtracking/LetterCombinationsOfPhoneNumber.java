...
public class LetterCombinationsOfPhoneNumber {
	...
	private static void assist(String digits, String digitsback,
			Map<Character, String> hm, String temp, List<String> res) {
		...
		for (int i = 0; i < digits.length(); i++) {
			...
			for (int j = 0; j < button.length(); j++) {
				...
				temp = temp + letter;
				assist(digits.substring(i + 1, digits.length()), digitsback,
						hm, temp, res);
